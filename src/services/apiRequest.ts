...
const _generateConfig = (configParams?: ConfigParamsI | undefined): AxiosRequestConfig | undefined => {
  const config: AxiosRequestConfig = {}
  if (configParams?.queryParams && Object.keys(configParams?.queryParams)?.length) {
...
export const get = async (params: GetParamsI): Promise<ApiResponseI> => {
  try {
    const response = await api.get(_generatePath(params.url), _generateConfig(params))
...
export const post = async (params: PostParamsI): Promise<ApiResponseI> => {
  try {
    const response = await api.post(_generatePath(params.url), _generateConfig(params))
    const { status, data } = response
    return { status, data, error: null }
  } catch (error) {
    if (error instanceof AxiosError) {
...
export const patch = async (params: PatchParamsI): Promise<ApiResponseI> => {
  try {
    const response = await api.patch(_generatePath(params.url), _generateConfig(params))
    const { status, data } = response
    return { status, data, error: null }
  } catch (error) {
    if (error instanceof AxiosError) {
...
export const put = async (params: PatchParamsI): Promise<ApiResponseI> => {
  try {
    const response = await api.put(_generatePath(params.url), _generateConfig(params))
    const { status, data } = response
    return { status, data, error: null }
  } catch (error) {
    if (error instanceof AxiosError) {
...
export const deleteMethod = async (params: DeleteParamsI): Promise<ApiResponseI> => {
  try {
    ;
