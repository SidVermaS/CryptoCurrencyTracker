...
export const fetchCryptoCurrencies = createAsyncThunk<CryptoCurrencyI[], void>('cryptoCurrencies/fetchAll', async (_, thunkAPI) => {
  console.log('111 br fetchCryptoCurrencies');
  
  const state = thunkAPI.getState() as RootStateI
  const response = await getCryptoCurrencies({ vs_currency: state.currencies.selectedFiatCurrency.id })
  if (response?.error) {
